...
function AdminPanel() {
    ...
    useEffect(() => {
        onAuthStateChanged(auth, (usr) => {
            if (usr) {
                isUserActive(usr).then((isActive) => {
                    if (isActive) { 
    ...
    return (
        <>
            ...
            <div className="container">
                ...
                <button className="btn handler btn-outline-success mb-2 mx-1" onClick={handleUnblock}>Unblock 🔓</button>
                <button className="btn handler btn-outline-danger mb-2 mx-1" onClick={handleDelete}>Delete 🗑️</button>
            </div>

            <div style={{ width: '100vw' }} className="container text-center">
                <table style={{ borderRadius: '15px'}} className="table table-responsive table-striped table-hover table-bordered table-md">
                    <caption className="text-center">{userList.length} {userList.length === 1 ? 'user' : 'users'}</caption>
                    <thead className="table-light">
                        <tr>
                            <th className="col-sm-1">Select all <input type='checkbox' className="form-check-input users-checkbox" checked={isCheckedAll} onChange={handleSelectAllChange} /></th>
                            <th>Full Name</th>
                            <th>E-mail</th>
                            <th>Last login</th>
                            <th>Register Date</th>
